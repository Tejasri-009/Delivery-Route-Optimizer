...
def _build_path(previous: Dict[str, Optional[str]], end_node: str) -> List[str]:
    path: List[str] = []
    current = end_node
...
def run_dijkstra(graph, start_node, end_node):
    ...
    """
    # Priority Queue to store tuples of: (cumulative_distance, current_node)
    ...
    # Parent pointers to reconstruct shortest path at the end
    previous_nodes: Dict[str, Optional[str]] = {start_node: None}
    ...
    while min_heap:
        ...
        # Skip stale queue entries if a better distance was found later
        if current_dist > distances[node]:
            continue

        current_path = _build_path(previous_nodes, node)
        ...
        execution_steps.append({
            ...
            "Path Explored": " → ".join(current_path)
        })
        
        # Destination reached!
        if node == end_node:
            return current_dist, current_path, execution_steps
...
def build_network_graph() -> nx.Graph:
    graph = nx.Graph()
    for node, neighbors in DELIVERY_GRAPH.items():
